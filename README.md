...
- **부산대학교** | 개발자 인턴 (2026.01 ~ 2026.02) · 2개월
  - AI 관련 서비스의 운영/유지보수 지원 (기획 중심 업무)
...
> **문제의 원인을 끝까지 추적하고, 더 나은 방법을 고민하는 개발자입니다.**

실무에서는 사용자 증가로 발생한 서비스 지연 문제를 Frontend에 국한하지 않고 Python Backend까지 분석해 원인을 찾아 해결했습니다. 이 과정에서 개발자가 예상한 사용 시나리오와 실제 사용자의 행동에는 차이가 있을 수 있다는 것을 배웠고, 사용자의 실제 사용 맥락을 이해하는 것이 좋은 서비스를 만드는 데 중요하다고 생각합니다.

개인 프로젝트에서는 주어진 환경과 제약(서버리스 환경, DB 커넥션 등)을 고려해 적합한 기술과 구조를 직접 선택합니다. 문제를 분석할 때는 AI를 단순한 코드 생성 도구가 아니라 원인 분석과 기술적 대안 탐색을 돕는 보조 도구로 활용하되, 검증과 최종 판단은 코드와 공식 문서를 통해 직접 내립니다.

해결한 문제는 그대로 흘려보내지 않고 기술 블로그에 기록하며, **해결 과정과 기술적 판단을 다시 정리해 지식으로 전환하는 습관**을 만들어가고 있습니다.
